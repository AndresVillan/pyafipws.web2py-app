...
if not session.has_key("comprobante"): raise HTTP(500, "No especifica id de cbte.")
...
try:
    cbte_tmp = db(db.comprobante.id == session.comprobante).select().first()
    SERVICE = cbte_tmp.webservice
...
def crear_cuit_paises():
    ...
    if int(response["FEXGetPARAM_DST_CUITResult"]["FEXErr"]["ErrCode"]) == 0:
        ...
        db.dstcuit.insert(ds = "(Sin especificar)", cuit = 33693450239)
...
def autorizar():
    ...
    au_form = FORM(INPUT(_type="submit", _value="Autorizar"), _id="form_autorizar")
    
    if au_form.accepts(request.vars, session, formname = "autorizar_comprobante"):
        # autorizar el cbte
        pass
    elif au_form.errors:
    ...
    try:
        ...
        if SERVICE=='wsfe':
            ...
            if comprobante.tipocbte.cod in [11, 12, 13, 15]:
                items_iva = []
            else:
                items_iva =  [{'AlicIva': {
                            'Id': det["id"],
            ...
            try:
                nro_doc_tmp = comprobante.nro_doc.replace("-", "")
            except AttributeError:
                nro_doc_tmp = None
        ...
        db.xml.insert(request = client.xml_request, response = client.xml_response)        
        return dict(resultado = {"fault": "Error en el Cliente SOAP. Formato de respuesta inválido."}, pdf = None, au_form = au_form)

    except (AttributeError, ValueError, TypeError, KeyError), ee:
        db.xml.insert(request = client.xml_request, response = client.xml_response)
        return dict(resultado = {"fault": "Se produjo un error al procesar los datos del comprobante o los datos enviados son insuficientes."}, pdf = None, au_form = au_form)
    ...
    # return dict(au_form = au_form, resultado = actualizar, pdf = A('Guardar el comprobante en formato PDF', _href=URL(r = request, c="salida", f="guardar_comprobante", args=[comprobante,])))
    return dict(au_form = au_form, resultado = actualizar, pdf = None)
